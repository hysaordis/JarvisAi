...
public class OllamaLlmClient : ILlmClient
{
   ...
   private Message ParseOllamaResponse(ChatResponseStream response)
   {
   ...
   public async Task<Message> SendCommandToLlmAsync(List<Message> messages, CancellationToken cancellationToken)
   {
       try
       {
           ...
           // Use provided messages if available, otherwise load from store
           var conversationMessages = messages ?? await _conversationStore.GetAllMessagesAsync();

           // Convert input messages to Ollama format with safe Role parsing
           var ollamaMessages = conversationMessages.Select(m => new OllamaSharp.Models.Chat.Message
           {
           ...
           var chatRequest = new ChatRequest
           {
               ...
               Tools = toolsArray?.Any() == true ? toolsArray : null,
               Stream = false,
               KeepAlive = "5m"
           };

           _logger.LogInformation($"Sending request to Ollama with {ollamaMessages.Count} messages" + 
               (toolsArray?.Any() == true ? $" and {toolsArray.Count} tools" : " without tools"));
           ...
           await _conversationStore.SaveMessageAsync(responseMessage);

           return responseMessage ?? new Message
           {
               Role = "assistant",
       ...
       {
           _logger.LogError($"Error in SendToOllamaAsync: {ex.Message}");
           throw;
       }
   }
